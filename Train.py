...
def train(env_step=2, title=""):
    ...
    # directory
    directory = f"ratio{config.mec_max_ratio}_lap{train_lap_num}_"+ title + datetime.datetime.now().strftime("%Y-%m-%d-%H") + "/"
    ...
    #                                      config=config, reset_stre=[0., 1., 1.], test_traj=True)
    ...
    for sub_train in range(train_lap_num):
        ...
        Coupled_Model.mec2hpc_stre = 1.
        training_func(Coupled_Model, lapnum=2, config=config,
                      v_abs=0.01, dy=0.01,
    ...
    HPC_u_after, _, _, _, _ = testing_func(Coupled_Model=Coupled_Model, init_get_loc=0, get_view=1,
                                            config=config, reset_stre=[0., 5., 5.], test_traj=True)
...
def test(directory, env_step=2, prefix="", load_ckpt=None):
    ...
    # hpcC, hpc_max, mecC, loc = testing_func(Coupled_Model=Coupled_Model, config=config,
    #                                                 init_get_loc=init_get_loc, get_view=get_view,
    #                                                 reset_stre=[0., 5., 5.],
    #                                                 test_traj=True, test_module_name="mec_hpc")
    # data00 = {"hpcC":hpcC, "hpc_max":hpc_max, "mecC":mecC, "loc": loc}
    # np.save("./tmp/center_traj.npy", data00)
    ...
    u_mec, hpc_fr, I_mec, I_sen, loc = testing_func(Coupled_Model=Coupled_Model, config=config,
                                                    init_get_loc=1, get_view=get_view, reset_stre=[0., 5., 5.],
