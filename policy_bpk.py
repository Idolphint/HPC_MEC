...
class GridPolicy(nn.Module):
    def __init__(self, device, action_std_init=0.01, mec_module_num=7):
        ...
        #     nn.Conv2d(mec_module_num*2, 32, 3, padding=5, padding_mode='circular'),
        #     # SinActivation(),
        #     nn.MaxPool2d(3,1),
        #     nn.ReLU(),
...
class ReplayBuffer:
    def __init__(self, buffer_size, mec_module=7, mec_x_num=10, mec_y_num=10, writer=None, device='cpu'):
        ...
        self.gt_diff = torch.zeros((self.buffer_size,2)).to(self.device)
        self.index = 0
    ...
    def add_state(self, state, done, logp, value, r, a, diff):
    ...
    def get_buffer(self):
        ...
        diffs = self.gt_diff[loss_mask]
        return old_states, old_actions, old_logp, old_values, diffs
...
def simple_train_loc():  # TODO 测试完记得把conv in_c * 2
